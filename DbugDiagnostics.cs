...
public class DbugDiagnostics : MonoBehaviour
{
    ...
    void OnEnable() => StartCoroutine(BurnInCoroutine());
    public void StartColorBurnIn() => StartCoroutine(ColorCalibration());
    public void StartFullBurnIn() => StartCoroutine(BurnInCoroutine());

    private IEnumerator BurnInCoroutine()
    {
        // Initial warm-up message
        Dbug.Emphasis("SPWN Dbug Warming up. This will take a few seconds.");
        yield return OneSec;

        // title log (wait 1 second)
        Dbug.Emphasis("Burn-In Sequence 1 Beginning. [Default Dbug Logs]");
        yield return OneSec;
        ...
        // title log (wait 1 second)
        Dbug.Emphasis("Burn-In Sequence 2 Beginning. [Styled Dbug Logs]");
        yield return OneSec;
        ...
        // title log (wait 1 second)
        Dbug.Emphasis("Burn-In Sequence 3 Beginning. [Colored Dbug Logs]");
        yield return OneSec;

        yield return ColorCalibration();

        // title log (wait 1 second)
        Dbug.Emphasis("Burn-In Sequence 4 Beginning. [Special Dbug Logs]");
        yield return OneSec;
        ...
        // title log (wait 1 second)
        Dbug.Emphasis("Burn-In Sequence 5 Beginning. [Status Dbug Logs]");
        yield return OneSec;
        ...
        Dbug.Succeeded("This is a Succeeded message.");
        yield return HalfSec;

        Dbug.Failed("This is a Failed message.");
        yield return HalfSec;

        // title log (wait 1 second)
        Dbug.Emphasis("Final Burn-In Sequence. [Countdown to Crash()]");
        yield return OneSec;

        // Countdown to crash
        Dbug.Emphasis("Crashing in 3...");
        yield return OneSec;
